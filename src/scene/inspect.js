  ...
  function inspect(e, { fromClick = false } = {}) {
    ...
    if (fromClick && isRiver && hydroStation) {
      ...
      tooltip.show(e.clientX, e.clientY, {
        ...
        status: hydroStation.discharge_m3s?.status ?? "LIVE",
        source: hydroStation.discharge_m3s?.source ?? "1D_HYDRAULIC_PROFILE",
        confidence: hydroStation.confidence ?? "MEDIUM",
        datumStatus: hydroStation.provenance?.datum ?? "VERIFIED_DATUM",
        roughnessStatus: hydroStation.provenance?.roughness ?? "VERIFIED",
        stationRecord: hydroStation,
      });
      return;
    }
