...
KML_PATH = os.path.join(HERE, "data", "mula_mutha_river.kml")
...
def main():
    ...
    out_profile = os.path.join(HERE, "naditwin", "rivers", "mula_mutha", "chainage_profile.json")
    with open(out_profile, "w") as f:
        json.dump(result, f, indent=2)

    print(f"Wrote {len(result)} chainage stations to {out_profile}")
    ...
    out_poly = os.path.join(HERE, "naditwin", "rivers", "mula_mutha", "reach_polygon.json")
    with open(out_poly, "w") as f:
        json.dump([[round(lo, 7), round(la, 7)] for lo, la in poly_wgs], f)
    print(f"Wrote reach polygon to {out_poly}")
